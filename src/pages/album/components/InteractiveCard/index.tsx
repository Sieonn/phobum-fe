...
export const InteractiveCard = memo(function InteractiveCard({ 
  ...
  useEffect(() => {
    ...
    const debouncedResize = debounce(checkSizes, 250);
  ...
  }, [image.image_url, isLoaded, imageUrl]);
  ...
  // 상호작용 처리 핵심 로직
  const handleInteraction = useCallback((clientX: number, clientY: number) => {
    if (!cardRef.current || !isInteracting.current) return;
    
    // 터치/마우스 이동 간격이 너무 짧으면 스킵 (모바일 성능 최적화)
    const now = Date.now();
    if (isMobile && now - lastTouchTime.current < 16) { // 약 60fps
      return;
    }
    ...
    // 이전 위치와 너무 가까우면 업데이트 건너뛰기 (추가 최적화)
    ...
    const sizeMultiplier = Math.min(
      isMobile ? 2 : 1, 
      rect.width / (isMobile ? 120 : 200)
    );

    // 모바일에서는 좀 더 부드러운 회전을 위해 값 제한
    const baseRotation = isMobile ? 20 : 15;
    const maxRotation = isMobile ? 25 : 20;
    
    // 회전 각도 계산 - 모바일에서는 살짝 감소시켜 더 안정적으로
    const rotateY = clamp((x - centerX) / centerX * baseRotation * sizeMultiplier, -maxRotation, maxRotation);
    const rotateX = clamp((centerY - y) / centerY * baseRotation * sizeMultiplier, -maxRotation, maxRotation);
    ...
    rafRef.current = requestAnimationFrame(() => {
      // 부드러운 변환을 위해 CSS transition이 적용된 transform 사용
      setTransform(`perspective(1000px) rotateX(${rotateX}deg) rotateY(${rotateY}deg) ${isSmallCard || isMobile ? 'scale(1.02)' : ''}`);
      
      // Shine 효과 적용 (빛 반사)
      if (shineRef.current) {
        const shineSize = isMobile ? '70%' : '80%';
        const shineOpacity = isMobile ? 0.25 : 0.3;
        shineRef.current.style.background = `radial-gradient(circle at ${x}px ${y}px, 
          rgba(255,255,255,${shineOpacity}), transparent ${shineSize})`;
      }
  ...
  const resetCardStyles = useCallback(() => {
    ...
    if (prismRef.current) {
      prismRef.current.style.opacity = '0';
    }
  }, []);
